...
import { StyleSheet, Text, View, Image, Alert, FlatList, Platform, StatusBar, Picker, TouchableOpacity } from 'react-native';
    ...
    render(){
        ...
        <View style={styles.container}>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'flex-end' }}>
                <Text style={[iOSUIKit.largeTitleEmphasized, styles.title]}>Search</Text>
                <TouchableOpacity onPress = {()=>{this.dropDown && this.dropDown.show();}}>
                <View style = {{ paddingRight:10, flexDirection: 'row', alignSelf: 'flex-end'}}>
                        <ModalDropdown ref={(el) => {this.dropDown = el}}
                        options={["%1h", "%24h", "%7d"]}
                        defaultIndex={parseInt(this.props.timePeriod)}
                        defaultValue={this.props.timeCategory}
                        textStyle = {{fontWeight:'bold', textAlign: 'right', color: 'white'}}
                        adjustFrame={style => {
                        ...
                        onSelect={(idx, category) => { this.props.changePeriod(idx)}}
                        />
                        <Text style={{alignSelf: 'flex-end'}}> <Ionicons name={'md-arrow-dropdown'} color='white' size={20} /></Text>
                        </View>
                </TouchableOpacity>
            </View>
